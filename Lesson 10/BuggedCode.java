...
public class BuggedCode {
    public static void main(String[] args) {
        ...
        int total = 0;
        int min = 100; //BUG ONE- INT MIN SHOULD BE 100 NOT 0
        int max = -1;
        final int COUNT = 4;
        ...
        for (int i = 1; i <= COUNT; i++) {
            ...
            if (s < 0 || s > 100) { //BUG TWO- CHANGE && TO ||
                if (s < 0)
                    s = 0;
                if (s > 100)
                    s = 100;
        ...
        // calculate average.
        double avg = (double) total / COUNT; //BUG THREE- CAST TO DOUBLE BEFORE DIVIDING

        // Did the student pass?
        boolean passed = false;
        if (avg >= 60)
              //BUG FOUR- REMOVE THE SEMICOLON THAT WAS HERE
        {
            passed = true;
        }
